...
# @gin.configurable
class tm700GraspingProceduralEnv(gym.Env):
  ...
  def reset(self):
    ...
    self._tm700.applyAction([0, 0, -0.3, 0, 0.3])
    for i in range(100):
      pybullet.stepSimulation(physicsClientId=self.cid)
  ...
  def _get_simple_observation(self):
    ...
    state = pybullet.getLinkState(
        self._tm700.tm700Uid, self._tm700.tmEEffectorIndex,
        physicsClientId=self.cid)
  ...
  def _step_continuous(self, action):
    ...
    state = pybullet.getLinkState(self._tm700.tm700Uid,
                                  self._tm700.tmEEffectorIndex,
                                  physicsClientId=self.cid)
