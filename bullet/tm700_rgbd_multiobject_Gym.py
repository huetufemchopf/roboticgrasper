...
class tm700_rgbdmultiobj_gym(tm700_possensor_gym):
  ...
  def _step_continuous(self, action):
    ...
    reward = self._multipleobjreward()
  ...
  def _multipleobjreward(self):
    ...
    for uid in self._objectUids:
      ...
      closestPoints1 = p.getClosestPoints(uid, self._tm700.tm700Uid, 10, -1,
                                          self._tm700.tmFingerIndexL)
      closestPoints2 = p.getClosestPoints(uid, self._tm700.tm700Uid, 10, -1,
                                          self._tm700.tmFingerIndexR)  # id of object a, id of object b, max. separation, link index of object a (base is -1), linkindex of object b
      numPt = len(closestPoints1)
    #print(numPt)
      if (numPt > 0):
  ...
  def _get_random_object(self, num_objects, test):
    ...
    """
    if test:
      urdf_pattern = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'shrunk01_random_urdfs/*0/*.urdf')
    else:
      urdf_pattern = os.path.join(os.path.dirname(os.path.realpath(__file__)), 'shrunk01_random_urdfs/*[1-9]/*.urdf')
